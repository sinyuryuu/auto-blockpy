...
Blockly.Python.finish = function (code) {
  ...
  Blockly.Python.nameDB_.reset(); // acbart: Don't actually inject initializations - we don't need 'em.
...
function BlockMirrorTextEditor(blockMirror) {
  ...
  this.codeMirror.on("change", function (cm, change) {
    if (_this.blockMirror.configuration.imageMode) {
      ...
      _this.updateImages(cm, change.from.line, lastLine); //console.log("Text content changed");
...
function BlockMirrorBlockEditor(blockMirror) {
  ...
  this.loadBlocklyCSS(); //這裡是Blockly 原生的Workspace
  // Inject Blockly
...
BlockMirrorBlockEditor.prototype.toolboxPythonToBlocks = function (toolboxPython) {
  ...
  return toolboxPython.map(function (category) {
    ...
    var colour = BlockMirrorTextToBlocks.COLOR[category.colour]; //工具箱頭部產生

    var header = "<category css-icon =\"".concat(category.CSS, "\" name=\"").concat(category.name, "\" colour=\"").concat(colour, "\"");
...
BlockMirrorTextToBlocks.prototype.convertStatement = function (node, full_source, parent) {
  try {
    ...
    var extractedSource = this.getSourceCode(arrayMin(heights), arrayMax(heights)); //console.error(e);
...
TOOLBOX_CATEGORY.custominput = {
  ...
  name: "輸入/輸出",
  colour: "TEXT",
  blocks: ['""', "input()", "print()", "print(___)", "print(___,___)", "print(___,___,___)", "print(___,sep = '')", "print(___,end = '')", "''.format(___)", "___.replace('', '')", "___.split('')", "___.strip()", "''.join(___)", "___.lstrip()", "___.rstrip()", "___.upper()", "___.lower()", "___.isdigit()"]
};
...
TOOLBOX_CATEGORY.custommath = {
  ...
  name: "數學",
  colour: "MATH",
  blocks: ["0", "-___", "___ + ___", "___-___", "___ * ___", "___ / ___", "___ % ___", "___ ** ___", "___ // ___", "abs(___)", "round(___)"]
};
TOOLBOX_CATEGORY.customiteration = {
  CSS: "fas fa-undo fa-2x fa-fw",
  name: "疊代",
  colour: "CONTROL",
  blocks: ["for ___ in range(___, ___, ___): pass", "while ___: pass", "range(___, ___, ___)", "break", "continue", "pass", "True", "False"]
};
...
TOOLBOX_CATEGORY.DECISIONS = {
  name: "Decisions",
  colour: "LOGIC",
  blocks: ["if ___: pass", "if ___: pass\nelse: pass", "___ < ___", "___ and ___", "elif__:\n    pass", "else__:\n    pass", "___ and ___", "break", "not ___"]
};
...
BlockMirrorBlockEditor.prototype.TOOLBOXES = {
  ...
  '<toolboxlabel name="工具箱" colour="#A3321A" css-label="customLabel"></toolboxlabel>', TOOLBOX_CATEGORY.SEP, TOOLBOX_CATEGORY.custom, TOOLBOX_CATEGORY.customtype, TOOLBOX_CATEGORY.custominput, TOOLBOX_CATEGORY.custommath, TOOLBOX_CATEGORY.customiteration, //TOOLBOX_CATEGORY.TEST,
  '<category css-icon="las la-exclamation-circle la-3x green" name="判斷" categorystyle="logic_category">', '<block type="controls_if"></block>', //原生的if
  //'<block type="controls_whileUntil"></block>',
  // '<block type="ast_If"></block>', //單獨使用blpckpy的if
  '<block type="ast_Compare">', '<field name="OP">Eq</field>', "</block>", '<block type="ast_Compare">', '<field name="OP">NotEq</field>', "</block>", '<block type="ast_Compare">', '<field name="OP">Gt</field>', "</block>", '<block type="ast_Compare">', '<field name="OP">Lt</field>', "</block>", '<block type="ast_BoolOp">', '<field name="OP">And</field>', "</block>", '<block type="ast_BoolOp">', '<field name="OP">Or</field>', "</block>", "</category>", TOOLBOX_CATEGORY.custommlist, TOOLBOX_CATEGORY.custommdict, TOOLBOX_CATEGORY.custommtuple, TOOLBOX_CATEGORY.custommset],
  //******************************************************
  ...
  full: [TOOLBOX_CATEGORY.VARIABLES, {
    ...
    name: "Logic",
    colour: "LOGIC",
    blocks: ["___ if ___ else ___", "___ == ___", "___ < ___", "___ in ___", "___ and ___", "not ___", "not ___"]
...
Blockly.Blocks['ast_If'] = (_Blockly$Blocks$ast_I = {
  init: function init() {
    this.orelse_ = 0;
    this.elifs_ = 0;
  ...
  updateShape_: function updateShape_() {
    var latestInput = "BODY"; //else 只會有一個
    ...
    for (var i = 0; i < this.elifs_; i++) {
      if (!this.getInput("ELIFTEST" + i)) {
        ...
        this.appendValueInput("ELIFTEST" + i).setCheck(null).appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF' + i); // 修改此處

        this.appendStatementInput("ELIFBODY" + i).setCheck(null).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF_BODY' + i) // 修改此處
    ...
    if (this.orelse_ && !this.getInput("ORELSETEST")) {
      ...
      if (ko == 0) {
        ...
        this.appendDummyInput("ORELSETEST").appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSE); // 修改此處

        this.appendStatementInput("ORELSEBODY").setCheck(null).appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN); // 修改此處
...
}, _defineProperty(_Blockly$Blocks$ast_I, "mutationToDom", function mutationToDom() {
  var container = document.createElement("mutation");
  container.setAttribute("orelse", this.orelse_);
  container.setAttribute("elifs", this.elifs_);
  return container;
}), _defineProperty(_Blockly$Blocks$ast_I, "domToMutation", function domToMutation(xmlElement) {
  this.orelse_ = "true" === xmlElement.getAttribute("orelse");
  this.elifs_ = parseInt(xmlElement.getAttribute("elifs"), 10) || 0;
  this.updateShape_();
}), _defineProperty(_Blockly$Blocks$ast_I, "decompose", function decompose(workspace) {
...
}), _defineProperty(_Blockly$Blocks$ast_I, "compose", function compose(containerBlock) {
  var clauseBlock = containerBlock.nextConnection.targetBlock(); // Count number of inputs.

  this.elifs_ = 0;
  this.orelse_ = false;
  var valueConnections = [null];
  var statementConnections = [null]; //console.log("Orelse Count (after compose):", this.orelse_);
...
Blockly.Blocks['ast_If_elif'] = {
  init: function init() {
    this.elifCount_ = 0; //this.appendValueInput('ELIFTEST0').appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF);//block text and input()
    ...
    this.valueConnection_ = null; //this.appendDummyInput().appendField("elif"); //積木下方的文字
  },
  mutationToDom: function mutationToDom() {
    if (!this.elifCount_) {
      return null;
  ...
  domToMutation: function domToMutation(xmlElement) {
    ...
    for (var i = 1; i <= this.elifCount_; i++) {
      var inputIf = this.appendValueInput('ELIFTEST' + i).appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF' + i); // 修改此處

      var inputDo = this.appendStatementInput('ELIFDO' + i).setCheck(null).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF_BODY' + i) // 修改此處
      .appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN);
    }

    if (this.elifCount_) {
      this.appendDummyInput('TAIL').appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_TAIL') // 修改此處
  ...
  compose: function compose(containerBlock) {
    ...
    this.elifCount_ = 0; // Rebuild the block's optional inputs.
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        case 'ast_If_elif':
          this.elifCount_++;
          var ifInput = this.appendValueInput('ELIFTEST' + this.elifCount_).appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF' + this.elifCount_); // 修改此處

          var doInput = this.appendStatementInput('ELIFDO' + this.elifCount_).setCheck(null).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELIF_BODY' + this.elifCount_) // 修改此處
          .appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN); // Reconnect any child blocks.
...
Blockly.Blocks['ast_If_orelse'] = {
  init: function init() {
    this.setTooltip("或是"); //this.appendStatementInput('ORELSEDO').setCheck(null).appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN); // 積木的形狀
  ...
  domToMutation: function domToMutation(xmlElement) {
    ...
    for (var i = 1; i <= this.orelseCount_; i++) {
      var inputDo = this.appendStatementInput('ORELSEDO' + i).appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSE).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELSE_BODY' + i); // 修改此處
    }
  },
  decompose: function decompose(workspace) {
    var containerBlock = workspace.newBlock('ast_If_orelse');
    containerBlock.initSvg();
    var connection = containerBlock.nextConnection; // Remove any extra else block from mutator
  ...
  compose: function compose(containerBlock) {
    ...
    this.orelseCount_ = 0; // Rebuild the block's optional inputs.
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        case 'ast_If_orelse':
          this.orelseCount_++;
          var doInput = this.appendStatementInput('ORELSEDO' + this.orelseCount_).appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSE).appendField(new Blockly.FieldLabel(' ', 'blocklyTextEmph'), 'LABEL_ELSE_BODY' + this.orelseCount_); // 修改此處
...
Blockly.Python["ast_If"] = function (block) {
  // Test
  var test = "if " + (Blockly.Python.valueToCode(block, 'TEST', Blockly.Python.ORDER_NONE) || Blockly.Python.FALSE) + ":\n"; // Body:
...
Blockly.Python['ast_Name'] = function (block) {
  // Variable getter.
  var code = Blockly.Python.nameDB_.getName(block.getFieldValue("VAR"), Blockly.Variables.NAME_TYPE);
...
Blockly.Python["ast_Assign"] = function (block) {
  ...
  if (block.targetCount_ === 1 && block.simpleTarget_) {
    targets[0] = Blockly.Python.nameDB_.getName(block.getFieldValue("VAR"), Blockly.Variables.NAME_TYPE);
  } else {
...
Blockly.Python["ast_AugAssign"] = function (block) {
  ...
  if (block.simpleTarget_) {
    target = Blockly.Python.nameDB_.getName(block.getFieldValue("VAR"), Blockly.Variables.NAME_TYPE);
  } else {
...
Blockly.Python['ast_Attribute'] = function (block) {
  // Text value.
  var value = Blockly.Python.nameDB_.getName(block.getFieldValue('VALUE'), Blockly.Variables.NAME_TYPE);
...
[["Parameter", "Parameter", "", false, false], ["ParameterType", "Parameter with type", "", true, false], ["ParameterDefault", "Parameter with default value", "", false, true], ["ParameterDefaultType", "Parameter with type and default value", "", true, true], ["ParameterVararg", "Variable length parameter", "*", false, false], ["ParameterVarargType", "Variable length parameter with type", "*", true, false], ["ParameterKwarg", "Keyworded Variable length parameter", "**", false], ["ParameterKwargType", "Keyworded Variable length parameter with type", "**", true, false]].forEach(function (parameterTypeTuple) {
  ...
  Blockly.Python["ast_Function" + parameterType] = function (block) {
    var name = Blockly.Python.nameDB_.getName(block.getFieldValue("NAME"), Blockly.Variables.NAME_TYPE);
    var typed = "";
...
Blockly.Python["ast_FunctionDef"] = function (block) {
  // Name
  var name = Blockly.Python.nameDB_.getName(block.getFieldValue("NAME"), Blockly.Variables.NAME_TYPE); // Decorators
  ...
  for (var i = 0; i < block.handlersCount_; i++) {
    ...
    if (level !== BlockMirrorTextToBlocks.HANDLERS_CATCH_ALL) {
      ...
      if (level === BlockMirrorTextToBlocks.HANDLERS_COMPLETE) {
        clause += " as " + Blockly.Python.nameDB_.getName(block.getFieldValue('NAME' + i), Blockly.Variables.NAME_TYPE);
      }
    }
